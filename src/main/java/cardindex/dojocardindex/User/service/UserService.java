    ...
    public void exportActiveUsersToCSV(HttpServletResponse response) {
        ...
        try {
            ...
            activeUsers.forEach(user -> writer.printf("%s;%s;%s;%s;%s;%d;%s;%s;%.1f;%.1f;%s;%s;%s;%s;%s;%d;%d;%d;%s;%s;%s%n",
                    ...
                    user.getUserPhone() != null ? user.getUserPhone() : "-",
                    user.getIsCompetitor() != true ? "Не" : "Да",
                    user.getHeight(),
                    user.getWeight(),
                    user.getMedicalExamsPassed() != null ? user.getMedicalExamsPassed().format(dateFormatter) : "",
                    user.getContactPerson() != null ? user.getContactPerson() : "-",
                    user.getContactPersonPhone() != null ? user.getContactPersonPhone() : "-",
                    user.getContactPersonEmail() != null ? user.getContactPersonEmail() : "-",
                    user.getReachedDegree().getDescription() != null ? user.getReachedDegree().getDescription() : "не е посочена",
